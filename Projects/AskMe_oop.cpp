...
void referesh_data_base(map<string,map<string,vector<int>>>&mp_ques, map<int, vector<tuple<int, string, string, string>>>&threads_mp, map<int, pair<string, string> >&mp_ids, int &new_q_ids, int &thread_q_ids, string &username)            //done
{
...
struct database //model
{
    ...
    void sign_up() //done
    {
        ...
        referesh_data_base(mp_ques,threads_mp,mp_ids,new_q_ids,thread_q_ids,username);
...
struct controller
{
    ...
    controller(database &_db,map<string,
               map<string,vector<int>>>&mp_ques_,
               map<int, pair<string, string> >&mp_ids_,
               map<int, vector<tuple<int, string,
               string, string>>>&threads_mp_,int &new_q_ids_,int &thread_q_ids_,string &username_,bool &account_): db(_db), mp_ques(mp_ques_), mp_ids(mp_ids_),
          threads_mp(threads_mp_), new_q_ids(new_q_ids_), thread_q_ids(thread_q_ids_),
          username(username_), account(account_){
...
struct system1
{
    ...
    system1(database &_db, controller _controller1): db(_db), controller1(_controller1){
...
void menu(int choice){
        ...
        if(!db.account)
        {
            ...
            case 2:
                db.sign_up();
                referesh_data_base(db.mp_ques,db.threads_mp,db.mp_ids,db.new_q_ids,db.thread_q_ids,db.username);
                break;
            default:
        ...
        else
        {
            ...
            {
            case 1:
                controller1.print_questions_to_me();
                break;

            case 2:
                controller1.print_questions_from_me();
                break;
            ...
            case 6:
                controller1.list_sys_users();
                break;
            case 7:
                controller1.system_feed();
                break;
            case 8:
                controller1.logout(db.account);
                break;
...
int main()
{
    database db;
    db.load_data();
    controller cont(db,db.mp_ques,db.mp_ids,db.threads_mp,db.new_q_ids,db.thread_q_ids,db.username,db.account);
    system1 sys(db,cont);
    while(!EXIT)
        sys.run();
...
//the plan is to make the database first then assign its variables with reference to other structs
